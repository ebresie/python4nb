...
public class PythonPlatformManager implements Serializable {
    ...
    ///////// The below is based on nbPython code as basis
...
//    private static PythonPlatformManager instance;
    ...
    private PythonPlatform getBundledPlatform() {
        ...
        // TODO: If included then leverage Jython possibly included in package
        ...
        if (jythonInstall == null ||  !jythonInstall.exists()) {
            return null;
        }
        ...
        List<String> list = discoverJythonClasspath((String) platform.getInterpreterCommand());
...
//        }
    ...
    public PythonPlatform findPlatformProperties(String cmd, String id) throws PythonException{
        ...
        try{
            ...
            String cmdArgs = "";  // argument to confirm version
            ...
            /* TODO figure out usage of "InstalledFileLocator" 
             * assume it's getting from properties someplace to identify paths
            or applicable details*/

            File info = InstalledFileLocator.getDefault().locate(
                 "python_info.py", "org.apache.netbeans.modules.python4nb.editor", false);
//                 "platform_info.py", "org.netbeans.modules.python4nb", false);
//                 "platform_info.py", "org.netbeans.modules.python.core", false);

// TODO: Link up with "python_info.py resources to poll for python details
            if (info != null ) { 
                pye.setScript(info.getAbsolutePath()); 
            } else {
                pye.setScript(null);  // TODO: Should this be a specific script, null, or ""?
                // no python_info script, try to get using --version argument
                cmdArgs = "--version";
            }
            if(cmdArgs.length() > 0) {
                pye.setCommandArgs(cmdArgs);
            }
            /* TODO: Determine if this needs to be true or false; started as 
            false made true for debugging may need to revert back to false */
...
//            pye.setShowControls(true);
//            pye.setShowInput(true);
//            pye.setShowWindow(true);
//            pye.setShowProgress(true);
//            pye.setShowSuspended(true);
            ...
            if(value == 0){
                ...
                // TODO: Need to get output for the properties file 
                ...
                LOGGER.log(Level.INFO, "** python.command=" + command);
                ...
                if (name == null) {
                    // try to determine based on folder name
                    // TODO: determine if this is good alternative or return null
                    name = Paths.get(pye.getWorkingDirectory()).getFileName().toString();
//                      return null;
                }
                if (name == null) {
                    name = pye.getDisplayName();  // TODO: Find unique name
               }
                LOGGER.log(Level.INFO, "platform.name=" + name);
                ...
                /* TODO: Consider determining pathString not available maybe 
                ...
                // save discovered platform to platforms by platfor id
                platforms.put(platform.getId(), platform);
            }else{
...
//            throw ex;
    ...
    public synchronized void autoDetect() {
        ...
        try {
            ...
            // TODO: Decide if want to include a "bundled python" (i.e. jython)
            PythonPlatform deflt = getBundledPlatform();
            if (deflt != null) {
                defaultPlatform = deflt.getId();
                platforms.put(defaultPlatform, deflt);
            ...
            for(String path : ad.getMatches()){
                LOGGER.log(Level.INFO, "** Finding platform properties for " + path);
